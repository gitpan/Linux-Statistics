...
sub DiskStats {
   ...
   if (open DISK,'<',$file{diskstats}) {
      while (<DISK>) {
         if (/^\s+(\d+)\s+(\d+)\s+(\w+)\s+(\d+)\s+\d+\s+(\d+)\s+\d+\s+(\d+)\s+(\d+)\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+$/ && $4 && $6) {
            for my $x ($disk{$3}) {
               $x->{Major}          = $1;
               $x->{Minor}          = $2;
               $x->{ReadRequests}   = $4;
      ...
      while (<DISK>) {
         tr/A-Z/a-z/;
         next unless /^\s+(\d+)\s+(\d+)\s+\d+\s+(\w+)\s+(\d+)\s+\d+\s+(\d+)\s+\d+\s+(\d+)\s+(\d+)\s+\d+\s+\d+\s+\d+\s+\d+\s+\d+$/ && $4 && $6;
         for my $x ($disk{$3}) {
            $x->{Major}          = $1;
            $x->{Minor}          = $2;
            $x->{ReadRequests}   = $4;
...
sub FileStats {
   ...
   @fstat{qw(Dentries Unused AgeLimit WantPages)} = (split /\s+/, <DENT>)[0..3];
   close DENT;
